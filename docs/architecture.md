...
- **`Leaderboard.tsx`**: Nested routes under `/leaderboard/*`; passes **`studentId`** when the user has a participation profile. **Students**: `#/leaderboard` → **`SchoolHighlights`**; `#/leaderboard/quiz` and `#/leaderboard/typing` with year filters including **Staff**. **Teachers/admins**: index → **`StudentLeaderboard`** (Wall of Fame); **Quiz** and **Typing** tabs available; **`StudentQuizLeaderboard`** / **`StudentTypingLeaderboard`** with **Staff** filter for teacher scores.
- **`ValuesLearning.tsx`**: The "Values Lab" section — available to **students** and **staff** (linked `students/{id}` profile). **Values Explorer** supports private reflections; **Pop Quiz** and **Speed Type** save scores to Firestore.
- **`components/typing/`**: Speed Type implementation — [`ValuesTypingGame.tsx`](../components/typing/ValuesTypingGame.tsx) (tab shell), [`TypingTest.tsx`](../components/typing/TypingTest.tsx) (solo engine), [`TypingRaceLobby.tsx`](../components/typing/TypingRaceLobby.tsx) / [`TypingRaceLive.tsx`](../components/typing/TypingRaceLive.tsx) (minute-aligned live races). Service layer: [`services/typingGame.ts`](../services/typingGame.ts); passage content: [`data/typingPassages.ts`](../data/typingPassages.ts).
- **`StudentPlanner.tsx`**: Calendar (Term/Month/Week), **My Tasks**, and **My Goals** — available to students and staff with a participation profile.
...
### Teacher Views
- **`TeacherConsole.tsx`**: The main staff workspace: **Award Stamps**; **Student attention** (fair-recognition dashboard: suggested sort modes, up to 20 students school-wide and up to 5 per year with a view switcher, optional search, links to each student’s Values Passport, total stamp counts — see [`TeacherAttentionPanel`](../components/TeacherAttentionPanel.tsx) and [`studentAttention`](../services/studentAttention.ts)); **Activity**; **Review Requests** (routed stamp requests — academic subjects to subject teachers, locations to homeroom teachers for the student's year; admins see all, teachers see their queue via [`nominationRouting.ts`](../services/nominationRouting.ts)); **Rewards**. Jumping to **Award** from the attention list can preselect a student and prefill subject/value from gaps.
...
- **`AdminConsole.tsx`**: A protected dashboard for Super Admins.
    - **Student directory**: Search by name or email; **sort** by **grade** (numeric from grade text, e.g. Year 7 → 7), **first name** (first token of full name), or **last login** (`lastLoginAt`), with tie-breakers as implemented in [`AdminConsole.tsx`](../components/AdminConsole.tsx). **Last login** column shows relative time (**Never** when unset), exact datetime on hover, and emerald highlight when within 14 days. **Row checkboxes** and a header **select all** (visible rows only) support **bulk Archive** and **bulk Restore**. **Show archived** toggles visibility of soft-archived students (muted rows and an **Archived** badge). Per-row actions: **Edit**, **Archive** or **Restore**, **Reset progress**, and **Delete permanently** (Firestore document removed; distinct from archive).
    - **Teacher Management**: Add/remove staff; **role** per row (`TEACHER` | `ADMIN`); **Subjects taught** and **Homeroom years** for stamp-request routing (edit via pencil icon). **Routing coverage** lists all academic and location subjects; **Routing gaps** warns when a subject or year level has no assigned teacher. Persisted via [`updateTeacher`](../services/dataService.ts) on the `teachers` collection.
...
### Settings (all roles)
- **`SettingsPage.tsx`**: Routed at `#/settings` (gear in the header). Sections differ by role; includes **email notification preferences** ([`EmailNotificationsSettings.tsx`](../components/EmailNotificationsSettings.tsx)), **avatar** customization ([`AvatarSettingsSection.tsx`](../components/AvatarSettingsSection.tsx)) when the user has a `studentId` (students and staff with a linked participation profile), and **feedback** entry ([`FeedbackSettingsSection.tsx`](../components/FeedbackSettingsSection.tsx)) where applicable.
- **Avatar flow**: [`AvatarSettingsSection`](../components/AvatarSettingsSection.tsx) loads the student doc (cache or Firestore), opens [`AvatarEditor`](../components/AvatarEditor.tsx), and saves via [`updateStudentAvatarConfig`](../services/dataService.ts). **Students** unlock **Randomize** at 1 stamp and full customization at 5; **Staff** (`grade: "Staff"`) get `forceFullCustomization` so they can set a leaderboard avatar without earning stamps. Avatars render on quiz/typing leaderboards through [`LeaderboardFace`](../components/leaderboard/LeaderboardFace.tsx).
...
- **Stamps waiting reminder**: Scheduled daily (16:00 Sydney). If a student has **≥ 5** stamps newer than `lastLoginAt` and has opted in (`unseenStampsEmailEnabled`), they receive a nudge email with a passport link. Deduped in `unseen_stamps_email_sent` per login period.
...
### 1. Service Layer (`services/dataService.ts`, `services/teacherEngagement.ts`, `services/studentAttention.ts`, `services/avatarUrl.ts`, `services/studentEngagement.ts`, `services/typingGame.ts`)
...
- **`avatarUrl.ts`**: **`resolveStudentAvatarUrl`** / **`defaultAvatarUrlForName`** so UI can fall back to a DiceBear URL when `Student.avatar` is missing or not a valid `https` image URL. Used with **`onError`** by [`LeaderboardFace`](../components/leaderboard/LeaderboardFace.tsx) on quiz and typing leaderboards.
- **`typingGame.ts`**: Speed Type — race timing, solo progress, score persistence, live race subscriptions, leaderboard fetch. Passage selection delegates to [`data/typingPassages.ts`](../data/typingPassages.ts).
...
- **Role-Based Access**:
    ...
    - **Teacher**: Identified if their email exists in the `teachers` collection. Access to Teacher Console, Values Development, Student Details. On login, [`ensureStaffParticipationStudent`](../services/dataService.ts) auto-creates or links a **`students/{id}`** doc (`grade: "Staff"`) so staff can use **Values Lab**, **My Planner**, and game leaderboards without losing teacher role.
    - **Admin**: Hardcoded bootstrap email and/or `teachers` collection with role `ADMIN`. Full access to Teacher Console plus **Admin Console**; same staff participation profile as teachers.
