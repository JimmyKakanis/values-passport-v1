...
- **Nav**: In [`App.tsx`](../App.tsx), the link label is **School** (Building icon) for **students** and **Leaderboard** (Bar chart) for **teachers and admins**; both target **`/leaderboard`**. The nested route [`Leaderboard.tsx`](../components/Leaderboard.tsx) passes **`studentId`** to the student view on both the index route and **`/leaderboard/year-groups`**.
...
- **Staff index route** (`#/leaderboard`): Unchanged **Wall of Fame** — [`StudentLeaderboard.tsx`](../components/leaderboard/StudentLeaderboard.tsx) (filters, podium, list); filters in [`leaderboard/LeaderboardShared.tsx`](../components/leaderboard/LeaderboardShared.tsx).
...
- **Podium + list** (staff only): **Top 3** on podium; list from rank **4** (student leaderboard).
